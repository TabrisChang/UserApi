...
public class GetUsersByBranchCodeUseCaseImpl implements GetUsersByBranchCodeUseCaseApi {
  ...
  public GetUsersResponseCommand execute(GetUsersRequestCommand requestCommand) {
    ...
    try {
      List<GetUserResponseCommand> users = userService.getAllUserByBranchCode(mapper.toUserDto(requestCommand)).stream()
          .map(mapper::toGetUserResponseCommand).toList();
      resp.setStatusCode(StatusCode.SUCCESS);
      resp.setUsers(users);
    } catch (Exception ex) {
      log.error("Get Users By Branch Code Error.", ex);
    }
